...
func engineUnitEmit(engine string, req spec.EngineUnitRequest) (json.RawMessage, error) {
	...
	if !IsUnitRunMode(string(c.RunMode)) || string(c.RunMode) == "quadlet" {
		// Quadlet is generated by podman's own generator (the caller emits it); this
		// servant produces the non-quadlet systemd-unit form only.
