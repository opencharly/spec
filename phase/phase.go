...
// bootstrap plugin; both are verb plugins invoked the normal way).
//
// This is the LIFECYCLE phase set (bootstrap/schema/load/build/runtime), DISTINCT from
// the step-phase enum (spec/spec/ir_enums.go's PhasePrepare/PhaseInstall/PhaseCleanup) —
// different concepts sharing the "phase" noun. It is a fabric slice of the spec contract
// module — pure Go string constants, no heavy deps (#55 Rule 2) — relocated from the
// github.com/opencharly/sdk root package, the SINGLE SOURCE for the phase vocabulary
// (R3): charly's package main aliases these, and a plugin's Describe declares its phase
// against them. charly core imports this slice INSTEAD of the sdk root; the sdk root
// keeps a thin re-export during cutover then is deleted.
package phase
