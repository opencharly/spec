...
// flat EmitTarget interface into a recursive dispatcher.
//
// Every IR-consuming EmitTarget today (the local deploy target, the external vm deploy,
...
//    NestedExecutor{
//        Parent: outerVmSSH,
//        Jump:   NestedJump{Kind: JumpContainerExec, Engine: "podman", Target: "mychild"},
//    }
//
...
type NestedJump struct {
	...
	// podman (the historical default). The engine is DATA here so
...
// isContainerKind reports whether a JumpKind enters a container. One transport
// arm now (the engine is data), kept as a predicate so call sites read clearly.
func isContainerKind(kind JumpKind) bool {
	return kind == JumpContainerExec
}
...
// environment.
func (n *NestedExecutor) RunUser(ctx context.Context, script string, opts spec.EmitOpts) error {
...
func (n *NestedExecutor) RunBuilder(ctx context.Context, opts spec.BuilderRunOpts) ([]byte, error) {
	if n.Parent == nil {
...
func (n *NestedExecutor) PutFile(ctx context.Context, localPath, remotePath string, mode uint32, ownerRoot bool, opts spec.EmitOpts) error {
	if n.Parent == nil {
		return fmt.Errorf("NestedExecutor: nil Parent")
	}
...
func (n *NestedExecutor) GetFile(ctx context.Context, remotePath string, asRoot bool, opts spec.EmitOpts) ([]byte, error) {
	if n.Parent == nil {
		return nil, fmt.Errorf("NestedExecutor: nil Parent")
	}
...
// JumpContainerExec, a curated allowlist of session-
