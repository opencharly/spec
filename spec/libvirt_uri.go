...
// For remote hypervisors the caller builds the net.Conn itself by:
//
...
//
// Home note (FLOOR-legal leaf): PURE string parsing over the spec SSHTarget
// value type — relocated from sdk/vmshared (#55 vmshared Bucket B) so the
// kernel-floor charly check-graphics endpoint (check_graphics_endpoint.go)
// parses a libvirt URI without a vmshared import; vmshared keeps a re-export
// forwarder for its plugin/test callers. stdlib + spec value types only; the
// live go-libvirt transport that consumes the parsed target stays in the VM
// plugin / vmshared behavior layer.
