...
// `charly deploy del <name>`: the verb, the name, and the ONE valid skip-confirmation flag. Every
...
//
// The flag is `--assume-yes`, NOT `--yes`/`--force`: the command:deploy plugin's `charly deploy del`
...
// source-less `charly deploy from-box` deploy (both the pod path, candy/plugin-deploy/from_box_pod.go,
// and the kubernetes path, candy/plugin-deploy/deploy_from_box.go — R3, one function, two callers).
