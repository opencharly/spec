...
// install_step_view.go — the SINGLE bridge between the in-proc InstallStep IR and its
...
// the in-proc path cannot diverge. The step-IR round-trip test proves
// StepFromView(StepToView(s)) is DeepEqual to s for every kind.
//
// Relocated from sdk/deploykit (#55 step-4) so the type-switch over the concrete step
// vocabulary lives beside the vocabulary + the InstallStepView wire type — spec owns the
// whole IR.
...
func StepToView(step InstallStep) InstallStepView {
	v := InstallStepView{
...
func StepFromView(v InstallStepView) (InstallStep, error) {
...
func StepsToView(steps []InstallStep) []InstallStepView {
	if len(steps) == 0 {
		return nil
	}
	out := make([]InstallStepView, 0, len(steps))
	for _, s := range steps {
...
func stepsFromView(views []InstallStepView) ([]InstallStep, error) {
	if len(views) == 0 {
		return nil, nil
	}
...
// ---------------------------------------------------------------------------
// Small view↔struct mirror helpers.
// ---------------------------------------------------------------------------
...
func cacheMountsToView(mounts []CacheMountSpec) []CacheMountView {
	if len(mounts) == 0 {
		return nil
	}
...
func cacheMountsFromView(mounts []CacheMountView) []CacheMountSpec {
	if len(mounts) == 0 {
		return nil
	}
...
func artifactsToView(arts []ArtifactRef) []ArtifactView {
	if len(arts) == 0 {
		return nil
	}
...
func artifactsFromView(arts []ArtifactView) []ArtifactRef {
	if len(arts) == 0 {
		return nil
	}
