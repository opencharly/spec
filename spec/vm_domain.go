...
// VmDomainIdentity normalizes a deploy/deploy name into the per-deploy VM DOMAIN IDENTITY — the
...
// a deploy ref maps to its VM token (vm:arch → arch), and a direct `charly vm create <entity>` (whose
...
// SplitVmAddress detects the "vm:"-prefixed CLI ADDRESSING form (`charly deploy add/del
...
// ledger-identity) form strips it via this helper, one which needs the sanitized dc.Deploy
