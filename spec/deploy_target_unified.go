...
// command:deploy's Invoke(OpDeployDispatch). There is no per-kind dispatch
...
// dispatchNode pre-stage: the dispatch-merged DeployNode (the
...
type DeployContext struct {
	// Node is the dispatch-merged DeployNode. nil for a ref-based
...
// to an `charly deploy …` subcommand, so the dispatcher in ResolveTarget
...
// UpdateOpts parameterizes `charly deploy update`. On the wire Update
